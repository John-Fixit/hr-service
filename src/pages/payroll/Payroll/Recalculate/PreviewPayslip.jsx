...
const PayslipDrawer = ({ paySlipData }) => {
  const user = useMemo(() => paySlipData?.user || [], [paySlipData?.user]);
  const paymentData = useMemo(
    () => paySlipData?.paymentData || [],
    [paySlipData?.paymentData]
  );
  const deductionData = useMemo(
    () => paySlipData?.deductionData || [],
    [paySlipData?.deductionData]
  );
  ...
  const deductGross = useMemo(() => {
    const value = deductionData?.reduce(
      (a, b) => a + Number(b?.amount_to_pay),
      0
    );
    return value;
    ...
    >
      ...
      <div className="flex flex-col ">
        ...
        <span className="ml-auto text-lg py-1 text-black font-open-sans font-[600] px-2">
          PAY Month : {monthNames[user?.month - 1]} {user?.year}
        </span>
        <hr className="border border-gray-300" />
      </div>
            ...
            <div className="block w-full p-1 text-xs font-open-sans ">
              <div className="flex flex-col mb-10 font-open-sans">
                <div className="flex justify-between bg-[#cce6f7] px-1 font-open-sans gap-2">
                  ...
                  <div className="font-open-sans">{user?.empno}</div>
                </div>
                ...
                <div className="flex justify-between px-1 gap-2">
                  ...
                  <div className="font-open-sans">{user?.grade}</div>
                </div>
                <div className="flex justify-between bg-[#cce6f7] px-1 gap-2">
                  <div className="font-open-sans">STEP</div>
                  <div className="font-open-sans">{user?.step}</div>
                </div>
                <div className="flex justify-between px-1  gap-2">
                  <div className=" font-open-sans">ACCOUNT NUMBER</div>
                  <div className="font-open-sans">{user?.account_no}</div>
                </div>
                <div className="flex justify-between bg-[#cce6f7] px-1 gap-2 ">
                ...
                <div className="flex justify-between px-1 gap-2 ">
                  ...
                  <div className="font-open-sans">{user?.rank_name}</div>
                </div>
              </div>

              <div>
                {paymentData?.length > 0 && deductionData.length > 0 && (
                      ...
                      <table className="min-w-full  border-gray-300">
                        <thead className="">
                          <tr className="  text-[1rem] leading-normal font-open-sans">
                            ...
                            <th className="py-[0.1rem] font-open-sans text-right">
                              Amount To Pay (N)
                            </th>
                          </tr>
                        </thead>
