...
import { promotionCardsData} from "./data.js"

const PromotionsCard = () => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 my-4">
    {}
      {promotionCardsData?.map((prom, i)=>(
  <div
              key={i}
            ...
            >
              <div className="flex gap-2 items-center">
                <div
                  className={`rounded-full ${prom?.b_color} w-[50px] h-[50px] flex justify-center items-center`}
                >
                  <prom.icon
                    size={25}
                    className={`!font-bold ${prom?.t_color}`}
                  />
                </div>
                <span className="text-[15px] text-[rgb(39, 44, 51)] font-[500] leading-[19.5px]">
                 {prom?.type}
                </span>
              </div>
