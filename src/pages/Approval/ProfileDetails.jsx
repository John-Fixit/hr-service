          ...
          <ul className=" mt-2 text-[15px] flex flex-col space-y-3">
            {details?.data?.map((dt) => (
              <li
                ...
                className=" grid grid-cols-3  my-3   border-b pb-1 items-center"
              >
                <span className="text-[#444e4e] font-helvetica font-[500] text-[0.9rem] capitalize ">
                  {" "}
                  {dt?.ATTRIBUTE_NAME?.replace(/_/g, " ")}:
                </span>
                <div className="text-[#888888]  text-en w-full  max-w-sm fontbold font-profileFontSize ">
                  {dt?.CURRENT_VALUE ? (
                    <div
                      onClick={() =>
                        setImageModalOpen({
                      ...
                      className="w-[5rem] h-[5rem] rounded-full border-2 border-gray-200 overflow-auto bg-gray-50"
                    >
                      <img
                        src={
                          dt?.CURRENT_VALUE
                    ...
                    <span>{dt?.CURRENT_VALUE || "N/A"}</span>
                  )}
                </div>
                <div className="text-[#888888]  text-en w-full  max-w-sm fontbold font-profileFontSize ">
                  {dt?.NEW_NAME || dt?.NEW_VALUE ? (
                    <div
                      onClick={() =>
                        setImageModalOpen({
                          status: true,
                          file: dt?.NEW_NAME || dt?.NEW_VALUE,
                        })
                      }
                      className="w-[6rem] h-[6rem] rounded-full border-2 border-gray-200 overflow-auto bg-gray-50  cursor-zoom-in"
                    >
                      <img
                        src={
                          dt?.NEW_NAME
                    ...
                    <span>{dt?.NEW_NAME || dt?.NEW_VALUE || "N/A"}</span>
                  )}
                </div>
              </li>
            ))}
