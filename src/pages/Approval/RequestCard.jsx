...
const RequestCard = ({
  ...
  return (
    <div className="grid grid-cols-1 bottom-0 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 my-4">
      {requestHistory.map((request, i) => (
        <div
          key={i}
          className={`${
            request.id == requestStatus ? "bg-slate-100" : "bg-white"
          } py-4 cursor-pointer shadow-sm -top border border-[#dfe2e6] flex rounded-xl items-center justify-between px-4 gap-3`}
          style={{
            boxShadow:
