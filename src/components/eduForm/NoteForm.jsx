...
const NoteForm = ({ setIsOpen, restartStep }) => {
  ...
  const debouncedUpdateData = debounce((values) => {
    updateData({ educationNote: values?.notes });
  ...
  const { data: academics } = useGetProfile({
    path: API_URL.getAcademics
  });


  const package_id = academics?.data?.package_id;
 ...
 if (Object.keys(data.dataError).length > 0) {
      ...
      const json = {
        ...
        data_group: data_group?.length
          ? [...data_group, { ...data?.education, attachments: data?.attachments, notes }]
          : null,
        notes,
  ...
  const addToLIST = () => {
    if (Object.keys(data.dataError).length > 0) {
      ...
      updateData({ education: {}, educationNote: ""});
