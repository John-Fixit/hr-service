...
const NoteForm = ({ setIsOpen, restartStep }) => {
  ...
  const { updateData, data, updateDataGroup, data_group, resetState } =
    useFormStore();
    ...
    const debouncedUpdateData = debounce((values) => {
      updateData({ familyNote: values?.notes });
    ...
    if (Object.keys(data.dataError).length > 0) {
      ...
      const json = {
        ...
        data_group: data_group?.length
          ? [
              ...data_group,
              { ...data?.family, attachments: data?.attachments, notes },
            ]
          : null,
        notes,
  ...
  const addToLIST = () => {
    if (Object.keys(data.dataError).length > 0) {
      ...
      updateData({ family: {}, profile_picture_url: "", familyNote: "" });
