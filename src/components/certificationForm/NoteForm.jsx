...
const NoteForm = ({ setIsOpen, restartStep }) => {
  ...
  const debouncedUpdateData = debounce((values) => {
    updateData({ certificateNote: values?.notes });
  ...
  const { data: {data: certification} } = useGetProfile({ path: API_URL.getCertificate, key: "certification"})


  const package_id = certification?.package_id;
  ...
  const onSubmit = async (formData) => {
    ...
    updateData({ certificateNote: notes });
     if (Object.keys(data.dataError).length > 0) {
      ...
      const json = {
        ...
        data_group: data_group?.length
          ? [...data_group, { ...data?.certification, attachments: data?.attachments, notes }]
          : null,
        notes,
  ...
  const addToLIST = () => {
    if (Object.keys(data.dataError).length > 0) {
      ...
      updateData({ certification: {}, certificateNote: ""});
