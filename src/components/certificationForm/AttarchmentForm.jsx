...
const AttarchmentForm = ({ onNext, onPrev }) => {
  ...
  const {
    ...
    defaultValues: {
      attachmentss: data?.certificationAttachment?.attachmentss ?? [],
    },
  });
  ...
  const debouncedUpdateData = debounce((values) => {
    updateData({ certificationAttachment: { ...values } });
    ...
    <>
      <div className="">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="w-full shadow px-8 bg-white rounded">
            <div
          ...
          <div>
            {/* <ul className="">
              {getValues()?.attachmentss?.map((attachment, index) => (
