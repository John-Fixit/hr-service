...
import { useGetProfilePayslip } from "../../../API/profile";
...
const PayslipDrawer = ({ user }) => {
  ...
  const paymentData = useMemo(() => {
    ...
    const value = Object.values(val?.payslip)?.filter(
      (slip) => !slip?.pay_deduct
    );
  ...
  const paymentGross = useMemo(() => {
    const value = paymentData?.reduce(
      (a, b) => a + Number(b?.amount_to_pay),
      0
    );
  ...
  return (
    <div
      className="flex flex-col px-6 py-3 bg-white !font-open-sans min-h-[70vh]"
      ref={componentRef}
    >
      <div
        className="absolute inset-0 flex items-center justify-center pointer-events-none"
        style={{
          background: `url("/assets/images/NCAA.png") center/600px no-repeat`,
          opacity: 0.12, // faint watermark
        }}
      ></div>
      <div className="lg:hidden print:hidden flex items-center justify-center h-[50vh] text-lg text-center">
        Please view the payslip on Desktop for better experience
      </div>
      <div className="hidden lg:block print:block">
        <Button
          className={cn("fixed right-1 top-1", isPrinting ? "hidden" : "")}
          isIconOnly
          onClick={startprint}
        >
        ...
        <div className="flex gap-2 items-center justify-between mb-10">
          ...
          <div className="text-4xl text-black font-open-sans font-[500]">
            Payslip
          </div>
        </div>
        ...
        <div className="flex flex-col ">
          ...
          <span className="ml-auto text-lg py-1 text-black font-open-sans font-[600] px-2">
            PAY PERIOD : {monthNames[user?.month - 1]} {user?.year}
          </span>
          <hr className="border border-gray-300" />
        </div>
                  ...
                  {paymentData?.length > 0 && deductData.length > 0 && (
                    <div className="flex flex-col space-y-2 font-open-sans ">
                      <div className="text-sm font-[500]">
                        <h1 className="text-lg font-bold font-open-sans">
                          Gross Income
                        </h1>
                        <div className="border-t flex items-center justify-between font-open-sans">
                          ...
                          <span className="font-open-sans font-bold">
                            N {formatNumberWithComma(val?.gross_amount)}
                          </span>
                        </div>
                      </div>
                      <div className="text-sm font-[500]">
                        ...
                        <div className="border-t flex items-center justify-between">
                          <span className="font-open-sans">Amount</span>
                          <span className="font-open-sans font-bold">
                            N {formatNumberWithComma(val?.total_deduction)}
                          </span>
                        </div>
                      </div>
                      <div className="text-sm font-[500]">
                        ...
                        <div className="border-t flex items-center justify-between">
                          ...
                          <span className="font-open-sans font-bold">
                            ...
                            {formatNumberWithComma(
                              val?.gross_amount - val?.total_deduction
                            )}
                          </span>
                        </div>
                      </div>
                        ...
                        <table className="min-w-full  border-gray-300">
                          <thead className="">
                            <tr className="  text-[1rem] leading-normal font-open-sans">
                              <th className="py-[0.1rem] px-1  text-left font-open-sans">
                                Description
                              </th>
                              <th className="py-[0.1rem]  text-left font-open-sans">
                                Amount (N)
                              </th>
                              <th className="py-[0.1rem]  text-right font-open-sans">
                                YTD (N)
                              </th>
                            </tr>
                          </thead>
                          <tbody className=" text-sm  font-[400]  leading-3 ">
                            ...
                            <tr className="font-bold font-open-sans ">
                              <td className="font-open-sans font-bolder">
                                GROSS AMOUNT
                              </td>
                              <td className="font-open-sans">
                                N {formatNumberWithComma(paymentGross)}
                              </td>
                            </tr>
                          </tbody>
                        </table>
                        ...
                        <table className="min-w-full  border-gray-300">
                          <thead className="">
                            <tr className="  text-[1rem] leading-normal">
                              <th className="py-1 px-1  text-left font-open-sans">
                                Description
                              </th>
                              <th className="py-1  text-left font-open-sans">
                                Amount (N)
                              </th>
                              <th className="py-1  text-right font-open-sans">
                                YTD (N)
                              </th>
                            </tr>
                          </thead>
                          <tbody className=" text-sm  font-[400]  leading-3 ">
                            ...
                            <tr className="font-bold">
                              ...
                              <td className="font-open-sans">
                                N {formatNumberWithComma(deductGross)}
                              </td>
                            </tr>
                          </tbody>
                        </table>
                ...
                <div>
                  {paymentData?.length > 0 && deductData.length > 0 && (
                    <div className="flex justify-between bg-[#cce6f7] px-1 font-bold ">
                      ...
                      <div className="font-open-sans">
                        N {formatNumberWithComma(paymentGross - deductGross)}
                      </div>
                    </div>
                  )}
                </div>
