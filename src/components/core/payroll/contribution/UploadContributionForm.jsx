...
import { useGetContributionAllowance } from "../../../../API/payroll";
import { useGetAllPayrollStaff } from "../../../../API/payroll_staff";

const UploadContributionForm = ({
  control,
  watch,
  touchedFields,
  errors,
...
const StaffPaymentFields = ({ control, watch, touchedFields, errors }) => {
  const defaultRow = {
    staff_id: "",
    amount_to_pay: "",
  };
            ...
            return (
              <div key={field.id} className="border p-4 rounded-xl bg-white">
                <div className="flex justify-between">
                  ...
                  {fields.length > 1 && (
                    <span
                      className="cursor-pointer p-2 hover:bg-gray-100 transition-all duration-300 rounded-full"
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
                  <div className="flex flex-col flex-1">
                    ...
                    <Controller
                      name={`staff_payments.${index}.staff_id`}
                      control={control}
                      render={({ field }) => (
                        <>
                          <Select
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <Controller
                      name={`staff_payments.${index}.amount_to_pay`}
                      control={control}
                      render={({ field }) => (
                        <>
                          <InputNumber
                            aria-label="duration"
                            min={1}
                            placeholder="Enter amount_to_pay"
                            className="w-full"
                            {...field}
                            status={
                              touchedFields?.staff_payments?.[index]
                                ?.amount_to_pay &&
                              errors?.staff_payments?.[index]?.amount_to_pay
                                ? "error"
                                : ""
                            }
                          />
...
const ExportStaffLoanSampleButton = () => {
  const handleExportAsExcel = () => {
    ...
    exportExcel({ excelData, fileName: "Staff Payment Sample" });
  };
    ...
    <div className="flex justify-end items-end mb-2">
      ...
      >
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-lg hover:from-emerald-600 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg"
        >
          <Download className="w-4 h-4" />
          Export Payment Sample
        </button>
      </ConfigProvider>
    </div>
  );
...
UploadContributionForm.propTypes = {
...
export default UploadContributionForm;
