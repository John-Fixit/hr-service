...
import { useRecalculate13th, useSuspendStaffPayroll, useUpdateStaffPayroll } from "../../../../../API/payroll_staff";
...
const Recalculate13thMonthForm = ({handleClose, staffDetailData, refetchDetail}) => {
  ...
  const { mutateAsync: recalculate, isPending: loading } = useRecalculate13th();
  ...
  const onSubmit = async (d) => {
    ...
    const json = {
     staff_id: staffDetailData?.staff_id,
     company_id: userData?.data?.COMPANY_ID,
     payroll_id: staffDetailData?.payroll_run_id,
     percent,
    };
    ...
    if(!percent){
      ...
      try {
        ...
        const res = await recalculate(json);
              ...
              render={({ field }) => (
                <div>
                  <Input
                    ...
                    className="w-full"
                    onChange={(e) =>
                      onChange(e.target.value, "percent")
                    }
                  />
                  <span className="text-red-500">
                    {touchedFields?.percent &&
...
export default Recalculate13thMonthForm;

Recalculate13thMonthForm.propTypes = {
    handleClose: PropTypes.func.isRequired,
    staffDetailData: PropTypes.any,
