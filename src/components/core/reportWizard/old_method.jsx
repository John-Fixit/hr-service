...
//           staffNetRow[key] = `${staffKey} - Net Difference`;
//         } else {
//           staffNetRow[key] = '';
//         }
//       });
...
// const PayrollReportTable = ({ tableData, isLoading, is_grouped, is_staff = false, is_yearly = false, is_allowance = false, title }) => {
...
//         if (item.__isSummary || item.__isDifference || item.__isStaffNet) return true;
...
//         const dataRows = group.data.filter(item => !item.__isSummary && !item.__isDifference && !item.__isStaffNet);
...
//         const dataRows = group.data.filter(item => !item.__isSummary && !item.__isDifference && !item.__isStaffNet);
//         const summaryRows = group.data.filter(item => item.__isSummary || item.__isDifference || item.__isStaffNet);
...
//     const firstGroup = (is_grouped || is_staff) ? rowsData?.[0]?.data?.[0] : rowsData?.[0];
...
//       (key) => !tableHeadSet.has(key) && key !== '__isSummary' && key !== '__isDifference' && key !== '__isStaffNet' // Exclude summary, difference, and staff net markers
//     );
...
//           .filter(el => !el.__isSummary && !el.__isDifference && !el.__isStaffNet) // Exclude summary, difference, and staff net rows from total calculation
//           .reduce((subTotal, el) => {
...
// // console.log(paginatedData, is_staff)
//   // Updated table rendering section for staff data
// const renderStaffTableRows = () => {
//   console.log(paginatedData)
//   return paginatedData.flatMap((section, sectionIndex) => {
//     const rows = [];
    
//     // Staff Header Row
//     if (section.isStaffHeader) {
//       rows.push(
//         <TableRow 
//           key={`staff-header-${sectionIndex}`}
//           className="bg-blue-100 font-bold border-t-2 border-blue-300"
//         >
//           {generateTableHead()?.map((head, index) => (
//             <TableCell
//               key={`staff-header-cell-${index}`}
//               className="font-bold font-helvetica text-sm uppercase"
//             >
//               {index === 0 ? section.header : ""}
//               {index === generateTableHead()?.length - 1 ? (
//                 <span className="text-xs">({section.groupLength} records)</span>
//               ) : ""}
//             </TableCell>
//           ))}
//         </TableRow>
...
//     // Group Header Row
//     else if (section.isGroupHeader) {
//       rows.push(
//         <TableRow 
//           key={`group-header-${sectionIndex}`}
...
//               {index === 0 ? section.header : ""}
//               {index === generateTableHead()?.length - 1 ? (
//                 <span className="text-xs">({section.groupLength} items)</span>
//               ) : ""}
//             </TableCell>
//           ))}
//         </TableRow>
...
//         rows.push(
//           <TableRow
//             key={`data-row-${sectionIndex}-${rowIndex}`}
//             className={
...
//                 ? "bg-green-50 font-bold border-t-2 border-green-200"
//                 : ""
//             }
//           >
//             {generateTableHead()?.map((key, cellIndex) => (
//               <TableCell
//                 key={`data-cell-${sectionIndex}-${rowIndex}-${cellIndex}`}
...
//                     : rowData.__isStaffNet
//                     ? "font-bold text-green-800 opacity-100 py-4 text-sm"
//                     : "opacity-65"
//                 }`}
//               >
...
//   return (
//     <>
//         <span>{paginatedData?.lenght} dskjdla</span>
...
//                       is_staff ? (
//                           // Use the new renderStaffTableRows function
