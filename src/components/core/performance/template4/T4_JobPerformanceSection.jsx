...
const T4_JobPerformanceSection = ({
  register,
  setValue,
  getValues,
  formState,
  ...
  const [jobPerformance, setJobPerformance] = useState(
    watch("job_performance") || Array(10).fill("")
  );

  const [jobPerformanceAppraisalRanks, setJobPerformanceAppraisalRanks] =
    useState(watch("job_performance_appraisal_ranks") || Array(10).fill(""));

  //  console.log(getValues("job_performance_appraisal_ranks"))

  // Sync local state with react-hook-form on mount
  useEffect(() => {
    setValue("job_performance", jobPerformance, { shouldValidate: true, shouldTouch: true, });
  }, [setValue, jobPerformance]);

  useEffect(() => {
    setValue("job_performance_appraisal_ranks",jobPerformanceAppraisalRanks, {
      shouldValidate: true,
      shouldTouch: true,
    });
  ...
  const handleTextAreaChange = (index, value) => {
    ...
    setValue("job_performance", updatedAssessment, {
      shouldValidate: true,
      shouldTouch: true,
    });
  };
  ...
  const handleRanksTextAreaChange = (index, value) => {
    ...
    setValue("job_performance_appraisal_ranks", updatedAssessmentranks, {
      shouldValidate: true,
      shouldTouch: true,
    });
  };
            ...
            >
              <div className="flex gap-4">
                ...
                <Input.TextArea
                  ...
                  status={
                    formState.errors?.job_performance?.[0] === "" ? "error" : ""
                  }
...
T4_JobPerformanceSection.propTypes = {
...
export default T4_JobPerformanceSection;
