...
const CamElements = ({ refresh, save, onSubmit, onClose }) => {
  ...
  const [toReturnSuccess, setToReturnSuccess] = useState(true);
  const [isfailureScan, setIsfailureScan] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [IpInfo, setIpInfo] = useState(null);
  const { isOpen, onOpen, onClose: onDrop } = useDisclosure();
  ...
  const handleSuccess = async () => {
    ...
    // });
    if (res) {
      onSubmit({ capturedImage, loc: res?.data });
    }
    closeDown();
  };

  const handleFailure = () => {
    setIsfailureScan(true);
    setIsLoading(false);
    onOpen();
    toast.error("Your Verification failed. Please Retry", {
      position: "bottom-right",
      duration: "2000",
    });
  };
  ...
  return (
    <div className="absolute top-1 right-1  bg-white border rounded-lg p-2 z-[2]">
      <div className="w-full flex flex-col bg-purple-100  py-3">
          ...
          <div className="flex  items-start gap-2">
            ...
            {/* <div className="">
              <Checkbox
                size="sm"
                color="gray"
                isSelected={toReturnSuccess}
                onValueChange={() => {
                  setToReturnSuccess(!toReturnSuccess);
                }}
              />
            </div> */}
          </div>
        ...
        {capturedImage ? (
          <div className="relative">
            ...
            {hasSubmit ? (
              <ScanLine
                isFailed={isfailureScan}
                color={isfailureScan ? "red" : "green"}
              />
            ) : null}
          </div>
        ) : (
        ...
        <div className="flex gap-3 p-2 justify-end">
          ...
          {!isLoading && capturedImage && !isfailureScan && (
            <Button onClick={submit} color="success" disabled={!capturedImage}>
              Submit
            </Button>
          )}
        </div>
      ...
      <ErrorModal onClose={onDrop} isOpen={isOpen} />
    </div>
  );
};
