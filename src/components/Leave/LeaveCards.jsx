      ...
      {leaveHistory.map((leave) => (
        ...
        >
          <div className="flex gap-2 items-center">
            ...
            <span className="text-[15px] font-helvetica text-[rgb(39, 44, 51)] font-[400] leading-[19.5px]">
              {leave.label}
            </span>
          </div>
          <span className="text-[14px] font-helvetica leading-[19.5px] text-[rgba(39, 44, 51, 0.5)] font-[400] font-Roboto">
            {/* {leaveNo(leave?.id)?.length} */}
            {loading?.[leave?.id] ? (
              <StarLoader size={20} />
            ) : (
              leaveNo && leaveNo(leave?.id)?.length
            )}
          </span>
        </div>
      ))}
